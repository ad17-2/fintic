...
function getMonthTotals(month: number, year: number) {
  return db
    .select({
      ...
      totalExpenses: sql<number>`COALESCE(SUM(CASE WHEN ${transactions.type} = 'debit' AND (${categories.name} IS NULL OR ${categories.name} NOT IN (${sql.raw(ALLOCATION_CATEGORIES.map((c) => `'${c}'`).join(","))})) THEN ${transactions.amount} ELSE 0 END), 0)`,
      allocations: sql<number>`COALESCE(SUM(CASE WHEN ${transactions.type} = 'debit' AND ${categories.name} IN (${sql.raw(ALLOCATION_CATEGORIES.map((c) => `'${c}'`).join(","))}) THEN ${transactions.amount} ELSE 0 END), 0)`,
  ...
  const dailySpending = db
    ...
    .where(
      and(
        ...
        sql`(${categories.name} IS NULL OR ${categories.name} NOT IN (${sql.raw(ALLOCATION_CATEGORIES.map((c) => `'${c}'`).join(","))}))`
      )
    )
